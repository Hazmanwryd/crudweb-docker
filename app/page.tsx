    ...
    <div className="py-10 px-10 text-center">
        ...
        {people.map((person, index) => (
          <div key={index} className="text-center group">
            <img
              src={person.image}
              alt={person.name}
              className={`w-48 h-48 md:w-64 md:h-64 lg:w-80 lg:h-80 rounded-xl mx-auto mb-4 object-cover animate-bounce-up-down`}
              style={{
                animationDelay: `${Math.random() * 4}s`, // Delay acak untuk setiap gambar
              }}
            />
      ...
      <style jsx>{`
        ...
        .animate-bounce-up-down {
          animation: bounce-up-down 6s ease-in-out infinite; /* Durasi animasi diperpanjang menjadi 6 detik */
        }
      `}</style>
    </div>
  );
